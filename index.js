...
var cors = require('cors'); /** Middleware to enable CORS */
...
const store = MongoStore.create({
    ...
    crypto: {
        secret: secret /** Secret to encrypt session id according to OWASP recommendations*/
    }
});
...
/** Allowed frame sources (in iframe, for example) */
const frameSrcUrls = [
    "https://www.youtube.com"
];
...
app.use(express.static(path.join(__dirname, 'public'))); /** Allows access to the "public/" directory */
app.use(express.urlencoded({ extended: true })); /** Parse only urlencoded bodies with qs library */
app.use(methodOverride('_method')); /** Use "_method=[PUT|DELETE]" in action attribute of the form tag */
...
passport.use(new LocalStrategy(User.authenticate())); /** Set authentication and authorization strategy */
passport.serializeUser(User.serializeUser()); /** Store user data to the session */
passport.deserializeUser(User.deserializeUser()); /** Get user data from the session */
