 ...
 */
class MediaControl {
    constructor(videoFrame, videoBtns, videoContent, videoId, chaptersList, classPart, chaptersDropdowns) {
...
const getCtrlElements = (classPart) => {
    ...
    for (let i = 0; i < 3; i++) {
        chaptersDropdowns.push(document.getElementById(`${classPart}chaptersDropdown${i}`));
    }
        ...
        if (videoBtns[i]) {
            videoBtns[i].addEventListener('click', () => {
                ...
                let videoId = videoFrame.src
                    .split('/')[4]
                    .substring(0, videoFrame.src.split('/')[4].indexOf('?'));
                let chaptersList = document.getElementById(`${classPart}chaptersList${i}`);
