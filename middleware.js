...
module.exports.isLoggedIn = (req, res, next) => {
  if (!req.isAuthenticated()) {
    ...
    req.flash(
      'error',
      'Ops, parece que você precisa entrar na sua conta para acessar essa página.'
    );
    return res.redirect('/');
  }
  next();
...
// check if user is the author of the course dashboard they are trying to edit
module.exports.isKnowmiaCourseDashboardAuthor = async (req, res, next) => {
  const { id } = req.params; // get information from request parameters
...
// validate edit course dashboard request
module.exports.validateEditKnowmiaCourseDashboard = (req, res, next) => {
  const { error } = editKnowmiaCourseDashboardSchema.validate(req.body);
  if (error) {
    const message = error.details.map((el) => el.message).join(',\n');
    res.render('error', { message });
    throw new ExpressError(message, 400);
...
// validate edit class request
module.exports.validateEditKnowmiaClass = (req, res, next) => {
  const { error } = editKnowmiaClassSchema.validate(req.body);
  if (error) {
    const message = error.details.map((el) => el.message).join(',\n');
    res.render('error', { message });
    throw new ExpressError(message, 400);
...
const iterateAndSanitize = (obj) => {
  Object.entries(obj).forEach((entry) => {
    if (typeof entry[1] === 'object' && entry[1] !== null) {
      iterateAndSanitize(obj[entry[0]]);
    } else {
      if (entry[1].includes('$')) obj[entry[0]] = entry[1].replace('$', '_');
      if (entry[1].includes('{')) obj[entry[0]] = entry[1].replace('{', '[');
      if (entry[1].includes('}')) obj[entry[0]] = entry[1].replace('}', ']');
    }
  });
};

module.exports.mongoSanitize = (req, res, next) => {
  iterateAndSanitize(req.body);
  next();
};